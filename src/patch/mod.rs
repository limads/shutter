...
use crate::shape::point::PointStats;
...
impl Patch {
    ...
    pub fn radial_expansion(
        ...
        for (ix, px) in self.pxs.iter().enumerate() {
            ...
            let dist = crate::shape::point_euclidian_u16(*px, seed);
...
impl PatchGroup {
    ...
    pub fn segment(img : &[u8], ncols : usize, threshold : u8) -> Self {
        ...
        for (i, px) in img.iter().enumerate() {
            ...
            let c = i % ncols;
            if i == 0 {
                patches.push(MultiPatch::new((r, c), *px));
            } else {
    ...
    pub fn patches(&self) -> &[MultiPatch] {
        &self.patches[..]
    }
...
impl ColorClustering {
    ...
    pub fn calculate_from_pixels<'a>(pxs : impl Iterator<Item=&'a u8> + 'a + Clone, n_colors : usize, hist_init : bool) -> Self {
        ...
        let means = extract_mean_colors(&km);
        //let px_iter = win.pixels(px_spacing).map(|d| [*d as f64] );
        let px_iter = pxs.map(|d| [*d as f64] );
        let extremes = extract_extreme_colors(&km, px_iter);
